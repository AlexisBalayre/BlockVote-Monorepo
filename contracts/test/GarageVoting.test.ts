...
describe("GarageVoting Contract Test", () => {
  ...
  it("Should not allow a non-admin to set merkleTreeDepth", async () => {
    await expect(
      hpCore.setMerkleTreeDepth(users[0], garageVoting, BigNumber.from(1))
    ).to.be.reverted;
  });

  it("Should allow an admin to set merkleTreeDepth", async () => {
    await expect(
      hpCore.setMerkleTreeDepth(admins[0], garageVoting, BigNumber.from(1))
    ).to.not.be.reverted;
  });

  it("Should not allow a non-admin to set a new verifier contract", async () => {
    await expect(
      hpCore.setVerifierAddress(users[0], garageVoting, garageVoting.address)
    ).to.be.reverted;
  });

  it("Should allow an admin to set a new verifier contract", async () => {
    await expect(
      hpCore.setVerifierAddress(admins[0], garageVoting, garageVoting.address)
    ).to.not.be.reverted;
  });

  it("Should not allow a non-admin to set a new poll implementation contract", async () => {
    await expect(
      hpCore.setPollImplementationAddress(
        users[0],
        garageVoting,
        garageVoting.address
      )
    ).to.be.reverted;
  });

  it("Should allow an admin to set a new poll implementation contract", async () => {
    await expect(
      hpCore.setPollImplementationAddress(
        admins[0],
        garageVoting,
